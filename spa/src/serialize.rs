...
use crate::{
    spa_pod_types,
    value::{Choice, ChoiceEnum, ChoiceValue},
};

use crate::{
    value::PropertyFlags, value::Value, value::ValueArray, CanonicalFixedSizedPod, FixedSizedPod,
};
...
impl<O: Write + Seek> PodSerializer<O> {
    ...
    /// - type: The type of the pod, e.g. `spa_pod_types::INT` for a `spa_pod_int`
    ...
    pub fn serialize_string(self, string: &str) -> Result<SerializeSuccess<O>, GenError> {
        let cstr = CString::new(string)
            ...
            .into_bytes_with_nul();
        self.write_pod(cstr.len(), spa_pod_types::STRING, slice(cstr))
    }

    /// Serialize a `Bytes` pod.
    pub fn serialize_bytes(self, bytes: &[u8]) -> Result<SerializeSuccess<O>, GenError> {
        self.write_pod(bytes.len(), spa_pod_types::BYTES, slice(bytes))
    }
    ...
    pub fn serialize_struct(mut self) -> Result<StructPodSerializer<O>, GenError> {
        ...
        self.gen(Self::header(0, spa_pod_types::STRUCT))?;
    ...
    pub fn serialize_object(
        ...
        self.gen(Self::header(0, spa_pod_types::OBJECT))?;
    ...
    pub fn serialize_choice<T: CanonicalFixedSizedPod>(
        ...
        let (choice_type, values) = match &choice.1 {
            ChoiceEnum::None(value) => (spa_pod_types::CHOICE_NONE, vec![value]),
            ChoiceEnum::Range { default, min, max } => {
                (spa_pod_types::CHOICE_RANGE, vec![default, min, max])
            }
            ChoiceEnum::Step {
                default,
                min,
                max,
                step,
            } => (spa_pod_types::CHOICE_STEP, vec![default, min, max, step]),
            ChoiceEnum::Enum {
                default,
                alternatives,
            } => {
                ...
                (spa_pod_types::CHOICE_ENUM, values)
            }
            ChoiceEnum::Flags { default, flags } => {
                let mut values = vec![default];
                values.extend(flags);
                (spa_pod_types::CHOICE_FLAGS, values)
            }
        };

        let len: usize = 2 * 8 + values.len() * (T::SIZE as usize);

        self.gen(Self::header(len, spa_pod_types::CHOICE))?;
    ...
    pub fn serialize_pointer<T>(
        ...
        let mut written = self.gen(Self::header(len, spa_pod_types::POINTER))?;
...
impl<O: Write + Seek> StructPodSerializer<O> {
    ...
    pub fn end(self) -> Result<SerializeSuccess<O>, GenError> {
        ...
        serializer.gen(PodSerializer::header(self.written, spa_pod_types::STRUCT))?;
...
impl<O: Write + Seek> ObjectPodSerializer<O> {
    ...
    pub fn end(self) -> Result<SerializeSuccess<O>, GenError> {
        ...
        serializer.gen(PodSerializer::header(written, spa_pod_types::OBJECT))?;
...
impl PodSerialize for std::collections::HashMap<String, String> {
    fn serialize<O: Write + Seek>(
        &self,
        serializer: PodSerializer<O>,
    ) -> Result<SerializeSuccess<O>, GenError> {
        ...
        for (key, value) in self {
            serializer.serialize_field(key.as_str())?;
            serializer.serialize_field(value.as_str())?;
        }
        serializer.end()
    }
}
